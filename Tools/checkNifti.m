function checkNifti(thisPath, verbose)
%---------------------------------------------------------------------------------------------
...
%         end
%     else
        dispi(['checkNifti checks the nifti header of the file : ',file], verbose)
%       - check that freq_dim is not 0
        if ni.freq_dim==0
             warni('NOT OK - incorrect freq_dim')
        else
             dispi('OK - freq_dim is not null')
        end
%       - check that phase_dim is not 0
        if ni.phase_dim==0
             dispi('NOT OK - incorrect phase_dim')
        else
             dispi('OK - phase_dim is not null')
        end
%       - check that slice_dim is not 0
        if ni.slice_dim==0
            dispi('NOT OK - incorrect slice_dim')
        else
            dispi('OK - slice_dim is not null')
        end
%       - check the method relating ijk and xyz (see http://nifti.nimh.nih.gov/nifti-1/documentation/nifti1fields/nifti1fields_pages/qsform.html)
%               method 1 if qform_code = 0 and sform_code = 0
%               method 2 if qform_code = 1 and sform_code = 0
%               method 3 if sform_code = 1
        if (ni.qform_code == 0) && (ni.sform_code == 0)
            dispi('ijk to xyz relation is using method 1')
        end
        if (ni.qform_code > 0) && (ni.sform_code == 0)
            dispi('ijk to xyz relation is using method 2')
        end
        if ni.sform_code > 0
            dispi('ijk to xyz relation is using method 3')
        end
            %check that pixdim is correct
            if sum(ni.pixdim)==0 || numel(ni.pixdim)~=ni.ndim
                dispi('NOT OK - incorrect pixdim (or ndim) parameters')
            else
                dispi('OK - pixdim and ndim are coherent')
            end
            %- check that sto_xyz is not 0
            if sum(ni.sto_xyz)==0
                dispi('NOT OK - sto_xyz is null')
            else
                dispi('OK - sto_xyz is not null')
            end 
...
%             %check that pixdim is correct
%             if sum(ni.pixdim)==0 || numel(ni.pixdim)~=ni.ndim
%                 dispi('incorrect pixdim (or ndim) parameters - NOT OK but fine if you can check that qto_xyz is correct')
%             else
%                 dispi('pixdim and ndim - OK')
%             end
            %check that qfac is 1 or -1
            if ni.qfac~=1 && ni.qfac~=-1
                dispi('NOT OK - qfac - fine if you can check that qto_xyz is correct')
            else
                dispi('OK - qfac is 1 or -1')
            end
            % check that qoffset are not 0
            if ni.qoffset_x==0 || ni.qoffset_y==0 || ni.qoffset_z==0
                dispi('NOT OK - One of the qoffset_* is null - check that - fine if you can check that qto_xyz is correct')
            else
                dispi('OK - qoffset_* are not null')
            end
            %       - check that qto_xyz is equal to sto_xyz
%             if (ni.sto_xyz==ni.qto_xyz)>0.0001
%                dispi('unequal sto_xyz and qto_xyz - NOT OK')
%             else
%                dispi('sto_xyz = qto_xyz - OK')
%             end
            % check that qto_xyz is not 0
            if sum(ni.qto_xyz)==0
                dispi('NOT OK - incorrect qto_xyz')
            else
                dispi('OK - qto_xyz is not null')
            end
            %check that the qto_xyz matrix is well formed with qoffset values
            if ni.qto_xyz(1,4)~=ni.qoffset_x || ni.qto_xyz(2,4)~=ni.qoffset_y || ni.qto_xyz(3,4)~=ni.qoffset_z
                dispi('NOT OK - qto_xyz is well ill-formed or qoffset_* are wrong')    
            else
               dispi('OK - qto_xyz is well formed regarding qoffset_* values')    
            end
            %check that qto xyz matrix is formed correctly according to the pixdim values
            R = ni.qto_xyz;
            ...
            if any(([dx, dy, dz] - ni.pixdim(1:3))>0.0001)
                dispi('NOT OK - qto_xyz is well ill-formed or pixdim are wrong')   
            else
               dispi(['OK - qto_xyz is well formed regarding pixdim - with precision ', num2str(max([dx, dy, dz] - ni.pixdim(1:3)))])    
            end
        %end
        
            %       - check that qto_xyz is equal to sto_xyz
            if sum(sum(ni.sto_xyz-ni.qto_xyz))>0.001
               dispi('NOT OK - unequal sto_xyz and qto_xyz')
            else
               dispi('OK - sto_xyz = qto_xyz')
            end
%             if (ni.sto_xyz==ni.qto_xyz)>0.0001
%                dispi('unequal sto_xyz and qto_xyz - NOT OK')
%             else
...
%- check that slice_end is not 0 (it should be nb of slices -1)   
        if ni.slice_end==0
            dispi('NOT OK - incorrect slice_end')
        else
            dispi('OK - slice_end is not null')
        end
%       - check that slice_duration is not 0     
        if ni.slice_duration==0
            dispi('NOT OK - incorrect slice_duration')
        else
            dispi('OK - slice_duration is not null')
        end
        
        dispi(['qoffset_* values are ', num2str([ni.qoffset_x, ni.qoffset_y, ni.qoffset_z])])
        dispi('************************************************')
%    end
end
