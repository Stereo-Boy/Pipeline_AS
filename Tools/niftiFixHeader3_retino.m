function niftiFixHeader3_retino(dataDir)
% 07.01.15
% Kelly Byrne
   ...
   disp(niiFileList')
    ...
    for j=1:numel(niiFileList)
       ...
       if numel(ni.fname)>3 && strcmp(ni.fname(1:3),'epi') || strcmp(ni.fname(1:4),'gems')
            ...
            ni.freq_dim = 1; % x is 1, y is 2, z is 3 - in neurology, this is L <-> R; P <-> A; I <-> S - but this does not seem to really matter
            ni.phase_dim = 2; 
            ni.slice_dim = 3;
            ni.slice_end = ni.dim(ni.slice_dim)-1; %(number of slices-1)
