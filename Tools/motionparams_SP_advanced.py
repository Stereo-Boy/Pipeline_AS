...
if __name__ == "__main__": 
    ...
    #functions for rotation matrixes around each axis
     # I define 3 functions that create rotation matrices for going around the x, y, and z axes.  The 4th function
     # that I create just makes those 3 rotations matrices and then multiplies them in the correct order to
     # create one rotation matrix that is the combined information for all 3.
    ...
    #I first create this rotation matrix for each difference in timepoints.
    for i in range(len(R1_differences)):
        ...
        #then find the maximum change from that original axis in each direction (assuming the rotations are done approx around brain center, so that the maximal motion point is
        #the one further away from center (half of the average brain dimension)
    ...
    #and add (maximal) translational change from rotations to raw translational changes
    ...
    for movement in total_T1_differences:
        if movement>voxel_size[0]*voxelFactor:
            #plot vertical line
    ...
    for movement in total_T2_differences:
        if movement>voxel_size[1]*voxelFactor:
            #plot vertical line
            ax1.plot(np.array([index+0.5, index+0.5]),
                     np.array([ymin, ymax]),
                     'r--') #red dashed line
    ...
    for movement in total_T3_differences:
        if movement>voxel_size[2]*voxelFactor:
            #plot vertical line
            ax1.plot(np.array([index+0.5, index+0.5]),
                     np.array([ymin, ymax]),
                     'g--') #green dashed line
